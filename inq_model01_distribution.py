...
initial_prompt = (
    ...
    "아주 중요한 것이니까, 꼭 지켜줘. 채점 결과를 제공할 때 항목마다 줄바꿈을 해 가독성이 좋게 제시하세요. 특히 실험 과정의 채점 결과는 반드시 항목마다 줄바꿈하세요. 예: 실험 과정 채점 결과:\n\n 5. 독립변인 조절의 조건\n 6. 변인 통제\n 7. 준비물\n 8. 독립변인 조절 언급"
...
# Streamlit-based chatbot with image upload and formatted markdown feedback for scientific inquiry support
import streamlit as st
from openai import OpenAI
import os
import json
...
각 줄은 한 문장으로 유지하고, 각 항목 간에는 줄 간격을 충분히 두세요. 줄바꿈은 `\n` 또는 `<br>`을 사용하세요.
"""
...
# Page 3: Main interaction with formatting
def page_3():
    st.title("탐구 도우미 활용하기")
    st.write("탐구 도우미와 대화를 나누며 탐구를 설계하세요.")
...
# Page 4: Generate formatted summary and save
def page_4():
    st.title("탐구 도우미의 제안")
    if not st.session_state.get("summary_generated"):
        chat_history = "\n".join(
        ...
        prompt = f"""
        다음은 학생과 과학탐구 도우미의 대화 기록입니다:

        {chat_history}

        이 대화 내용을 바탕으로 아래와 같은 형식으로 탐구 내용과 피드백을 작성하세요:
    ...
    st.markdown(st.session_state.get("summary", "요약 없음"), unsafe_allow_html=False)
